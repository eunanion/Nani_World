...
/* MYSQL에서 프로그래밍이 가능하고
변수나 함수 등을 지정할수 있습니다
물론 JAVA, Python, CPP등의 프로그래밍과 달리
제한되는 점이 많지만, 이를 활용하는 경우가 빈번하기 때문에
먼저 변수 지정 및 출력부터 진행하겠습니다. */

-- <MySQL에서 변수 사용하기>
-- MySQL에서 변수를 지정할때는 SET @변수명 = 값; 문법 사용합니다
...
-- 변수에 지정된 갑을 출력할때는 SELECT @변수명;을 사용합니다
-- @myVar1을 출력해보세요.
...
-- SELECT * FROM usertbl limit @myVar2; (사용불가)
...
-- 저장된 구문의 ?에 값을 전달하기 위해 USING 전달값을 사용합니다.

-- <MySQL의 데이터형식과 형 변환>
-- 데이터 변환시 CAST(), CONVERT() 등의 함수를 이용해서
...
SELECT CAST('2022&01&08' as DATE);
SELECT CAST('2022/01/08' as DATE);
SELECT CAST('2022@01@08' as DATE);
SELECT CAST('2022!01!08' as DATE);
SELECT CAST('2022#01#08' as DATE);
...
SELECT '100' + '200'; -- 문자와 문자 연산 => 정수로 변환
...
-- IFNULL(수식1, 수식2)
-- 수식1이 NULL이 아니먄 수식1이 변환, 수식1이 NULL이면 수식2로 반환합니다.
...
-- 원래 값을 보려면 BLOB -> 우클릭 -> Open value in viewer -> txet 탭 선택
...
SELECT CHAR_LENGTH('sdsdsdsdsd');

-- CONCAT(문자열1,문자열2,.........);

SELECT CONCAT('가','나','다','라마바사아자차카타파하');
...
SELECT FORMAT(1234.23423423532523,3);

-- BIN(숫자), HEX(숫자), OCT(숫자)
-- 2진수      16진수     8진수
-- 정수로 10진수 숫자로 바꿔서 표현해줍니다alter
...
SELECT INSERT ('일이삼사오육칠팔구십',3,4,'@@@@@');

-- LEFT(문자열,길이), RIGHT(문자열, 길이)
-- 해당 문자열의 왼쪽, 오른쪽에서 문자열 길이만큼만 남깁니다.

SELECT LEFT('abcsdfgdsgsd',3), RIGHT('일이삼사오육칠팔구십',4);
...
SELECT TRIM(BOTH 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ'); -- 양쪽 ㅋ 모두 제거
SELECT TRIM(LEADING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ');
SELECT TRIM(TRAILING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ');
...
-- 시작위치부터 길이만큼의 문자를 변환합니다.
-- 길이를 생략하고 파라미터를 2개만 주면
-- 시작지점부터 끝까지 모든 문자를 반환합니다.

SELECT SUBSTRING('일이삼사오육칠팔구십',6,4);


SELECT SUBSTRING('일이삼사오육칠팔구십'FROM 6) as '숫자';
SELECT SUBSTRING('일이삼사오육칠팔구십'FROM 6 FOR 4 ) as '숫자';
...
INSERT INTO getUser VALUES ('비비디',30);


DELIMITER $$ 
CREATE PROCEDURE getUser()
BEGIN #프로시저 내부에서는 #으로 주석을 사용합니다. -- 로 할 경우 꼬이는 구간이 발생하기도함
   SELECT * FROM usertbl;
END $$
DELIMITER ;
CALL getUser();
...
-- hire_date 칼럼의 DATE 자료를 이용해 판단합니다.
use employees;
DELIMITER $$
CREATE PROCEDURE checkFiveYear()
      BEGIN
          ...
          SELECT hire_date INTO hireDATE
              FROM employees WHERE emp_no = 30000;
...
-- DROP PROCEDURE를 이용해 삭제한 후 다른 번호 직원도 체크해보세요.getUser
