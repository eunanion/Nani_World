...
-- SELECT 칼럼명 FROM 테이블명 ORDER BY 기준컬럼 정렬기준;
...
-- 정령시 동점인 부분을 처리하기 위해 ORDER BY 절을 두 개 이상 동시에 나열할수도 있습니다.
-- 아래 코드는 키로 내림차순을 하되, 동점이면 생년으로 나열됩니다

SELECT * FROM usertbl ORDER BY height ASC, birthyear DESC;
...
-- 지역 가나다라 순으로 나열해주시되, 만약 지역이 같다면 이름 오름차순으로 정렬해주세요.

SELECT * FROM usertbl ORDER BY addr DESC, user_name ASC;

-- DISTINCT는 결과물로 나온 칼럼의 중복값을 다 제거하고
-- 고유값을 남겨줍니다.
...
-- output창 limit 0.1000 < 1000개까지 보여주도록 나옴alter
-- cmd와는 다르게 1000개만 보여주도록 나옴 설정으로 바꿀수도 있지만
-- 워크벤치로 바꿀일은 거의없음

-- 테이블명 뒤에 limit 숫자; 가 오는 경우는 적은 개수만큼만
-- 결과창에 보여줍니다
-- 숫자를 하나만 적으면 자동으로 0번 자료부터 n개만 보여줍니다alter
SELECT * FROM employees limit 10;

SELECT * FROM employees ORDER BY hire_date DESC limit 10;

-- 일정 데이터셋을 먼저 얻어놓고 그 내부적으로 정렬시킬때는 서브쿼리를 사용합니다.
...
-- SELECT 칼럼명, 집계함수(칼럼명2).. FROM 테이블명 GROUP BY 묶을컬럼명;
...
SELECT user_id, sum(amount*price) as 'total' FROM buytbl GROUP BY user_id;

-- 각 유저별 구매 물품의 평균가를 구해보세요(총액 아님) 평균은 avg()로 처리합니다.

SELECT user_id, avg(price*amount) as 'avg' FROM buytbl GROUP BY user_id;

-- 자주쓰는 집계`함수 정리 (통계적 결과를 얻기 위해 사용)
-- AVG() 평균
-- MIN() 최소값
-- MAX() 최대값
-- COUNT() ROW개수
...
-- WHERE처럼 사용하시되 HAVING 이라고 조건식을 적어서 붙여보세요.

SELECT user_id, sum(price * amount) as '총구매액'
    FROM buytbl GROUP BY user_id HAVING sum(price * amount) > 250000;
    
    
-- 키가 평균보다 큰 사람을 화면에 출력해주세여
...
SELECT user_id, height as '평균보다 키큰사람'
    FROM usertbl GROUP BY user_id HAVING height > (SELECT avg(height) FROM usertbl);
...
-- 테이블 생성하기
-- 테이블명 : testtbl, id_int, user_name varchar(3),age int형식으로 테이블을 생성해주세요
...
INSERT INTO testtbl (user_id,user_name) VALUES (2,'김딜동');
-- 만약 컬럼명 지정을 안 했지만 age에는 null을 넣고 싶다면
INSERT INTO testtbl VALUES (3,'박딜동',null);
SELECT * FROM testtbl;
-- 만약 입력하는 컬럼(열)의 순서를 바꿔서 지정하는 경우는 (id, user_name,age 가 아닌 순으로 넣음)
-- 모든 컬럼에 값을 입력하는 상황이어도 컬럼명을 따로 기입해야 합니다.
INSERT INTO testtbl(user_name,user_age, user_id) VALUES ('채종훈',10,4);
